...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while(1):
            ...
            frames = self.load_frames(video_file)
            if frames is None or frames.shape[0]>500:
                continue
            ...
            if mel_spectrogram.shape[0] > frames.shape[0]*self.scale_factor:
                mel_spectrogram = mel_spectrogram[:frames.shape[0]*self.scale_factor]
    ...
    def reprocess(self, data, idxs):
        ...
        max_frame_len = max(video_lens)
        # print("Max frame len: ", max_frame_len)
        visual_emb = np.array(visual_emb)
        # print("Frames before pad: ", visual_emb.shape)
        data = np.zeros(( len(visual_emb), max_frame_len, visual_emb[0].shape[1], visual_emb[0].shape[2], visual_emb[0].shape[3]), dtype=visual_emb.dtype )
        
        for i in range(len(visual_emb)):
            num_frames = visual_emb[i].shape[0]
            data[i][:num_frames] = visual_emb[i]
        
        visual_emb=np.array(data).astype(np.float32)
        # print("Frames after padding: ", visual_emb.shape)
