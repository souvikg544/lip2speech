...
class TacotronSTFT(torch.nn.Module):
    ...
    #     return spec
    # def inv_mel_spec(self, mel, griffin_iters=60):
    #     # _stft = self.STFT
    #     mel = torch.stack([mel])
    #     mel_decompress = self.spectral_de_normalize(mel)
    #     mel_decompress = mel_decompress.transpose(1, 2).data.cpu()
    #     spec_from_mel_scaling = 1000
    #     print("MB: ", self.mel_basis)
    #     spec_from_mel = torch.mm(mel_decompress[0], self.mel_basis)
    #     spec_from_mel = spec_from_mel.transpose(0, 1).unsqueeze(0)
    #     spec_from_mel = spec_from_mel * spec_from_mel_scaling

    #     audio = griffin_lim(
    #         torch.autograd.Variable(spec_from_mel[:, :, :-1]), self._stft_fn, griffin_iters
    #     )
