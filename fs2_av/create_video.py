...
def generate(args, video_file, result_path):

    # print("Orig video file: ", video_file)

    fname = video_file.split("/")[-2] + "_" + video_file.split("/")[-1].split(".")[0]
    # print("Fname: ", fname)

    audio_file = os.path.join(args.audio_path, fname+".wav")
...
def load_data(args):
    ...
    prog_bar = tqdm(video_paths)

    
    for video in prog_bar:
