...
from utils.model import get_model_all, get_model_all_face, get_vocoder
...
def get_predictions(args, video_file, model, configs, text_dict, result_path):
    ...
    speaker_emb = np.array([np.load(speaker_emb_file)['ref'][0]])
    ...
    visual_emb = np.array([frames/255.])
    ...
    # print(pred_wav_fname, gt_wav_fname)
...
if __name__ == "__main__":
    ...
    # Get model
    model = get_model_all_face(args, configs, device, train=False)
    model = model.cuda()
