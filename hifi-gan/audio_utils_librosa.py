...
def _mel_to_linear_torch(mel_spectrogram, hparams, device):
    global _inv_mel_basis
    if _inv_mel_basis is None:
        _inv_mel_basis = np.linalg.pinv(_build_mel_basis(hparams))
    _inv_mel_basis = torch.from_numpy(_inv_mel_basis).reshape(1,_inv_mel_basis.shape[0], _inv_mel_basis.shape[1]).to(device)    
    print("Inv b: ", _inv_mel_basis.shape)
    print("Mel: ", mel_spectrogram.shape)
    # dot = bdot(_inv_mel_basis, mel_spectrogram)
    # dot = (_inv_mel_basis*mel_spectrogram).sum(-1)
    dot = torch.bmm(_inv_mel_basis, mel_spectrogram)
    print("Dot prod: ", dot.shape)
    return torch.maximum(torch.tensor(1e-10).to(device), dot)

def _build_mel_basis(hparams):
    assert hparams.fmax <= hparams.sampling_rate // 2
    return librosa.filters.mel(hparams.sampling_rate, hparams.n_fft, n_mels=hparams.num_mels,
