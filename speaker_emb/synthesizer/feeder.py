...
class Feeder:
	...
	def get_window(self, center_frame):
		...
		l, r = center_id - self._hparams.T//2, center_id + self._hparams.T//2 + 1
		if l < 0 or r > len(os.listdir(vidname)):
			# print(('lr not satisfied'))
			return None

		window_ids = range(l, r)
		window_fnames = []
		for frame_id in window_ids:
			frame = join(vidname, '{}.jpg'.format(frame_id))
			if not isfile(frame):
				# print('file {} missing'.format(frame))
				return None
