...
def _get_image_list(split, T=29):
    pkl_file = 'logs/filenames_{}.pkl'.format(split)
    if os.path.exists(pkl_file):
        with open(pkl_file, 'rb') as p:
            return pickle.load(p)
    else:
        vidlist = glob('/ssd_scratch/cvit/rudra/lrw_bigger_preprocessed/*/train/*/')
        train_filelist, test_filelist = [], []
        for vid in tqdm(vidlist):
            imgs = glob(vid + '*.jpg')
            if len(imgs) != T: continue

            if np.random.uniform() < .95: train_filelist.extend(imgs)
            else: test_filelist.extend(imgs)

        with open(pkl_file, 'wb') as p:
            pickle.dump(train_filelist, p, protocol=pickle.HIGHEST_PROTOCOL)
        with open(pkl_file.replace('_train', '_test'), 'wb') as p:
            pickle.dump(test_filelist, p, protocol=pickle.HIGHEST_PROTOCOL)
...
hparams = HParams(
    ...
    tacotron_teacher_forcing_start_decay=156000,
    # starting point of teacher forcing ratio decay. Relevant if mode="scheduled"
    tacotron_teacher_forcing_decay_steps=256000,
    ...
    eval_ckpt="logs/tacotron_model.ckpt-313000",
    T=25,
    overlap=10,
    mel_overlap=32,
    mel_step_size=80,
